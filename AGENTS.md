...
### 后端结构 (`/src/`)
- `adapters/`: 外部 API 适配器（如 GitHub, Follow API）。
...
### Service-Adapter 模式
- **Adapters**: 负责外部数据抓取和格式转换。必须继承 `BaseAdapter` 并实现 `fetchAndTransform` 方法。
...
2. **制定计划**: 
   ...
   - 在 `TaskService` 中注册新功能，并在 `initServices.ts` 中完成实例化。
3. **安全准则**:
   - **严禁**硬编码任何敏感信息。
...
### 添加新适配器 (New Adapter)
1. 在 `src/adapters/` 创建 `NewSourceAdapter.ts`。
2. 继承 `BaseAdapter`，实现 `name`, `category`, `fetchAndTransform`。
3. 在 `src/services/initServices.ts` (或类似初始化位置) 实例化并加入 `TaskService`。

...
export class GitHubStorage implements IStorageProvider {
  static metadata: StorageMetadata = {
    ...
    configFields: [
      ...
      { key: 'pathPrefix', label: '路径前缀', type: 'text', default: 'images' }
    ]
  };
  ...
  async upload(localFilePath: string, targetFilename: string): Promise<string | null> {
    try {
      ...
      try {
        ...
        const cdnUrl = `https://raw.githubusercontent.com/${this.owner}/${this.repo}/refs/heads/${this.config.branch}/${repoPath}`;
        return cdnUrl;
      } catch (e: any) {
        if (e.status === 422) { // File already exists
          return `https://raw.githubusercontent.com/${this.owner}/${this.repo}/refs/heads/${this.config.branch}/${repoPath}`;
        }
        throw e;
      }
    } catch (error: any) {
